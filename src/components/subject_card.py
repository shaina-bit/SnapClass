...
def subject_card(name, code, section, stats=None, footer_callback=None):
    html = f"""
       ...
       <h3 style="margin:0; color:#1e293b; font-szie: 1.5rem">{name}</h3>
       <p style="color:#64748b; margin:10px 0; >Code: <span style="background:#E03FF;color:#5865F2; padding:2px 8px; border-radius:5px;">{code} </span>| Section: {section}</p>
       """
    if stats:
       html += """
